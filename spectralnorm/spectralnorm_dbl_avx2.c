 ...
 *
 * modified to use AVX2 intrinsics on Aug 18, 2014
 */
...
/* already replaced with AVX2 intrinsics in inner loop */
...
//#endif
  __m256d err1;
  __m256d r1 = _mm256_broadcast_sd(&one);
  __m256d r2 = _mm256_broadcast_sd(&half);
  __m256d r3 = _mm256_broadcast_sd(&four);
  __m256d r4 = _mm256_broadcast_sd(&two);
  __m256d rf = _mm256_broadcast_sd(&flip);
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < n; j += 4) {
      ...
      /* A(i, j) by using FMA intrinsics */
      /* calc (i+j)(i+j+1) at first */
      //volatile __m256d a = _mm256_setr_pd(A(i,j), A(i,j+1), A(i,j+2), A(i, j+3));
      __m256d ymm_i_j_i_j_1 = _mm256_fmadd_pd(ymm_i_j, ymm_i_j, ymm_i_j);
      __m256d a = _mm256_fmadd_pd(ymm_i_j_i_j_1, r2, ymm_i_1);

      /* compute reciprocal of a by using FMA instinsics */
      __m256d ra = _mm256_cvtps_pd(_mm_rcp_ps(_mm256_cvtpd_ps(a)));
      ra = _mm256_mul_pd(_mm256_xor_pd(ra, rf), _mm256_fmsub_pd(ra, a, r4));
      ra = _mm256_mul_pd(_mm256_xor_pd(ra, rf), _mm256_fmsub_pd(ra, a, r4));

      sum = _mm256_fmadd_pd(b, ra, sum);
...
//#endif
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < n; j += 4) {
      ...
      /* calc (i+j)(i+j+1) at first */
      __m256d ymm_i_j_i_j_1 = _mm256_fmadd_pd(ymm_i_j, ymm_i_j, ymm_i_j);
      __m256d a = _mm256_fmadd_pd(ymm_i_j_i_j_1, r2, ymm_j_1);
