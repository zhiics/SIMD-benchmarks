...
void smvp_basic(int nodes, double ***A, int *Acol, int *Aindex,
		double **v, double **w);
void smvp_sse(int nodes, double ***A, int *Acol, int *Aindex,
		double **v, double **w);
...
void smvp_sse(int nodes, double ***A, int *Acol,
                int *Aindex, double **v, double **w) {
  int i;
  int Anext, Alast, col;
  double sum0, sum1, sum2;
...
//  __assume_aligned(w, 32);
  ...
  double sum_vec[4];
  __m128d sum0_vec, sum1_vec, sum2_vec, sum01_vec, sum20_vec, sum012_vec;
  __m128d Anext0_vec, Anext1_vec, Anext2_vec, v_vec, v0_vec, v1_vec, v2_vec, wi_vec, w_vec, wcol_vec;
  __m128d av0_vec, av1_vec, av2_vec, av_prod;
...
//    sum2 = A[Anext][2][0]*v[i][0] + A[Anext][2][1]*v[i][1] + A[Anext][2][2]*v[i][2];
        ...
        Anext0_vec = _mm_load_pd(A[Anext][0]);
        Anext1_vec = _mm_load_pd(A[Anext][1]);
        Anext2_vec = _mm_load_pd(A[Anext][2]);
        v_vec = _mm_load_pd(&v[i][0]);
        sum0_vec = _mm_mul_pd(Anext0_vec, v_vec);
	sum0 = _mm_cvtsd_f64(_mm_hadd_pd(sum0_vec, sum0_vec)) + A[Anext][0][2]*v[i][2];
        sum1_vec = _mm_mul_pd(Anext1_vec, v_vec);
	sum1 = _mm_cvtsd_f64(_mm_hadd_pd(sum1_vec, sum1_vec)) + A[Anext][1][2]*v[i][2];
        sum2_vec = _mm_mul_pd(Anext2_vec, v_vec);
	sum2 = _mm_cvtsd_f64(_mm_hadd_pd(sum2_vec, sum2_vec)) + A[Anext][2][2]*v[i][2];
...
//      sum2 += A[Anext][2][0]*v[col][0] + A[Anext][2][1]*v[col][1] + A[Anext][2][2]*v[col][2];
          ...
          Anext0_vec = _mm_load_pd(&A[Anext][0][0]);
          Anext1_vec = _mm_load_pd(&A[Anext][1][0]);
          Anext2_vec = _mm_load_pd(&A[Anext][2][0]);
          v_vec = _mm_load_pd(v[col]);

          sum0 += _mm_cvtsd_f64(_mm_mul_pd(Anext0_vec, v_vec)) + A[Anext][0][2] * v[col][2];
          sum1 += _mm_cvtsd_f64(_mm_mul_pd(Anext1_vec, v_vec)) + A[Anext][1][2] * v[col][2];
          sum2 += _mm_cvtsd_f64(_mm_mul_pd(Anext2_vec, v_vec)) + A[Anext][2][2] * v[col][2];

//      w[col][0] += A[Anext][0][0]*v[i][0] + A[Anext][1][0]*v[i][1] + A[Anext][2][0]*v[i][2];
//      w[col][1] += A[Anext][0][1]*v[i][0] + A[Anext][1][1]*v[i][1] + A[Anext][2][1]*v[i][2];
//      w[col][2] += A[Anext][0][2]*v[i][0] + A[Anext][1][2]*v[i][1] + A[Anext][2][2]*v[i][2];

          v0_vec = _mm_set1_pd(v[i][0]);
          v1_vec = _mm_set1_pd(v[i][1]);
          v2_vec = _mm_set1_pd(v[i][2]);
          wcol_vec = _mm_load_pd(w[col]);    //get the values for w[col][0], w[col][1] and w[col][2]

          av0_vec = _mm_mul_pd(Anext0_vec, v0_vec);
          av1_vec = _mm_mul_pd(Anext1_vec, v1_vec);
          av2_vec = _mm_mul_pd(Anext2_vec, v2_vec);
          av_prod = _mm_add_pd(_mm_add_pd(av0_vec, av1_vec), av2_vec);
          w_vec = _mm_add_pd(wcol_vec, av_prod);
          _mm_store_pd(w[col], w_vec);
