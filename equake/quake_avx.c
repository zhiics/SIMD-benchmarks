...
void smvp_basic(int nodes, double ***A, int *Acol, int *Aindex,
		double **v, double **w);
void smvp_avx(int nodes, double ***A, int *Acol, int *Aindex,
		double **v, double **w);
...
/* Time integration loop */
  ...
  start = ts.tv_sec + ts.tv_usec / 1000000.0;
  fprintf(stderr, "\n");
...
#if defined(AVX)
    smvp_avx(ARCHnodes, K, ARCHmatrixcol, ARCHmatrixindex, 
	       disp[dispt], disp[disptplus]);
#else
    smvp_basic(ARCHnodes, K, ARCHmatrixcol, ARCHmatrixindex, 
	       disp[dispt], disp[disptplus]);
#endif
...
//	                        (M[i][j] + Exc.dt / 2.0 * C[i][j]);
  ...
  gettimeofday(&ts, NULL);
  end = ts.tv_sec + ts.tv_usec / 1000000.0;

  fprintf(stderr, "Simluation time %f, %f, %f seconds\n", start, end, (end - start));
...
void smvp_basic(int nodes, double ***A, int *Acol, 
...
void smvp_avx(int nodes, double ***A, int *Acol,
                int *Aindex, double **v, double **w) {
  int i;
  int Anext, Alast, col;
  double sum0, sum1, sum2;
...
//  __assume_aligned(w, 32);
  ...
  double sum_vec[4];
  __m256d sum0_vec, sum1_vec, sum2_vec, sum01_vec, sum20_vec, sum012_vec;
  __m256d Anext0_vec, Anext1_vec, Anext2_vec, v_vec, v0_vec, v1_vec, v2_vec, wi_vec, w_vec, wcol_vec;
  __m256d av0_vec, av1_vec, av2_vec, av_prod;
...
//    sum2 = A[Anext][2][0]*v[i][0] + A[Anext][2][1]*v[i][1] + A[Anext][2][2]*v[i][2];
        ...
        wi_vec = _mm256_load_pd(w[i]);
        Anext0_vec = _mm256_load_pd(A[Anext][0]);
        Anext1_vec = _mm256_load_pd(A[Anext][1]);
        Anext2_vec = _mm256_load_pd(A[Anext][2]);
        v_vec = _mm256_load_pd(&v[i][0]);
        sum0_vec = _mm256_mul_pd(Anext0_vec, v_vec);
        sum1_vec = _mm256_mul_pd(Anext1_vec, v_vec);
        sum2_vec = _mm256_mul_pd(Anext2_vec, v_vec);

//      printf ("Anext = %d, Alast = %d\n", Anext, Alast);
...
//      sum2 += A[Anext][2][0]*v[col][0] + A[Anext][2][1]*v[col][1] + A[Anext][2][2]*v[col][2];
          ...
          Anext0_vec = _mm256_load_pd(&A[Anext][0][0]);
          Anext1_vec = _mm256_load_pd(&A[Anext][1][0]);
          Anext2_vec = _mm256_load_pd(&A[Anext][2][0]);
          v_vec = _mm256_load_pd(v[col]);

          sum0_vec = _mm256_add_pd(sum0_vec, _mm256_mul_pd(Anext0_vec, v_vec));
          sum1_vec = _mm256_add_pd(sum1_vec, _mm256_mul_pd(Anext1_vec, v_vec));
          sum2_vec = _mm256_add_pd(sum2_vec, _mm256_mul_pd(Anext2_vec, v_vec));

//      w[col][0] += A[Anext][0][0]*v[i][0] + A[Anext][1][0]*v[i][1] + A[Anext][2][0]*v[i][2];
//      w[col][1] += A[Anext][0][1]*v[i][0] + A[Anext][1][1]*v[i][1] + A[Anext][2][1]*v[i][2];
//      w[col][2] += A[Anext][0][2]*v[i][0] + A[Anext][1][2]*v[i][1] + A[Anext][2][2]*v[i][2];

//        v0_vec = _mm256_broadcast_sd(&v[i][0]);       //use broadcast or set1 instruction? 
          v0_vec = _mm256_set1_pd(v[i][0]);
//        v1_vec = _mm256_broadcast_sd(&v[i][1]);
          v1_vec = _mm256_set1_pd(v[i][1]);
//        v2_vec = _mm256_broadcast_sd(&v[i][2]);
          v2_vec = _mm256_set1_pd(v[i][2]);
          wcol_vec = _mm256_load_pd(w[col]);    //get the values for w[col][0], w[col][1] and w[col][2]

          av0_vec = _mm256_mul_pd(Anext0_vec, v0_vec);
          av1_vec = _mm256_mul_pd(Anext1_vec, v1_vec);
          av2_vec = _mm256_mul_pd(Anext2_vec, v2_vec);
          av_prod = _mm256_add_pd(_mm256_add_pd(av0_vec, av1_vec), av2_vec);
          w_vec = _mm256_add_pd(wcol_vec, av_prod);

          _mm256_store_pd(w[col], w_vec);

          Anext++;
    }

    sum01_vec = _mm256_hadd_pd(sum0_vec, sum1_vec);
    sum20_vec = _mm256_hadd_pd(sum2_vec, _mm256_setzero_pd());

    __m256d blend = _mm256_blend_pd(sum01_vec, sum20_vec, 0b1100);
    __m256d perm = _mm256_permute2f128_pd(sum01_vec, sum20_vec, 0x21);
    sum012_vec =  _mm256_add_pd(perm, blend);
    w_vec = _mm256_add_pd(wi_vec, sum012_vec);
