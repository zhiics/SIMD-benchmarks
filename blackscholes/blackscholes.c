...
#ifdef ENABLE_PAPI
	long long int pre_time = PAPI_get_real_usec();
...
#define LINESIZE 64
	...
	//added by zhi to get the time
	clock_gettime(CLOCK_MONOTONIC, &ts);
	end = ts.tv_sec * 1000000ULL + ts.tv_nsec / 1000;
	printf ("Total simulationi time is: %lld microseconds.\n", end - start);
	...
	fprintf (stdout, "The execution time of the application is: %lld microseconds\n", end - start);
	PAPI_shutdown();
    fclose(pFile);
#endif
